...
/**
 * copy code from android and change some code
 * @author littletli
 *
 */
public class ViewPager extends ViewGroup {
    ...
    protected static final int PAGE_SNAP_ANIMATION_DURATION = 450;
    ...
    // bounce effect
    private boolean mBounceEffectEnabled = true;
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        if (velocity > 0) {
            ...
            duration = PAGE_SNAP_ANIMATION_DURATION;
        }
        duration = Math.max(350,Math.min(duration, MAX_SETTLE_DURATION));
    ...
    // special use for dataSetChanged
    public void setCurItem(int curItem){
    	if (curItem > mAdapter.getCount()-1){
    		return;
    	}else{
    		mCurItem = curItem;
    	}
    ...
    // added for MultiTransformImageView
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mBounceEffectEnabled) {
                    	if (oldScrollX < leftBound || oldScrollX > rightBound) {
                    		deltaX = deltaX / 2;
                		}
                    	leftBound -= width/2;
                		rightBound += width/2;
     ...
     * {@link #endFakeDrag(float)} to complete the fake drag and fling as necessary.
     *
...
//        ev.recycle();
     ...
     * @see #fakeDragBy(float)
     */
    public void endFakeDrag(float xOffset) {
        if (!mFakeDragging) {
		...
		final long time = SystemClock.uptimeMillis();
		final MotionEvent ev = MotionEvent.obtain(mFakeDragBeginTime, time, MotionEvent.ACTION_UP, mLastMotionX, 0, 0);
		mVelocityTracker.addMovement(ev);
		ev.recycle();
        ...
        velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
        int initialVelocity = (int)(VelocityTrackerCompat.getXVelocity(
                velocityTracker, mActivePointerId)*1.2);

        long timeDelta = time - mFakeDragBeginTime;
        int simpleVelocity = 0;
        if (timeDelta > 0){
        	simpleVelocity = (int)(Math.abs(mLastMotionX)/timeDelta*1000);
        }

        mPopulatePending = true;
        if (Math.abs(initialVelocity) > mMinimumVelocity
        		|| simpleVelocity > mMinimumVelocity
	...
	public void fakeDragBy(float xOffset) {
		...
		float leftBound = Math.max(0, (mCurItem - 1) * widthWithMargin);
		float rightBound = Math.min(mCurItem + 1, mAdapter.getCount() - 1) * widthWithMargin;

		float scrollX = getScrollX();
		if (scrollX < leftBound || scrollX > rightBound) {
			xOffset = xOffset / 2;
		}
		mLastMotionX += xOffset;
		scrollX -= xOffset;
    ...
    public static  abstract class PagerAdapter {
         ...
         * page views.
         */
        public abstract void startUpdate(ViewGroup container);
         ...
         * page views.
         */
        public abstract void finishUpdate(ViewGroup container);
